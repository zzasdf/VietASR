...
from train_asr import add_model_arguments, get_params

import k2
import sentencepiece as spm
import torch
import torch.nn as nn
from asr_datamodule import AsrDataModule, FinetuneAsrDataModule
...
def decode_one_batch(
    ...
    # feature = batch["audio"].to(device)    # at entry, feature is (N, T, C)
...
def decode_dataset(
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        # cut_ids = [cut.id for cut in batch["cuts"]]                     # for FinetuneAsrDataModule               
        cut_ids = [cut.id for cut in batch["supervisions"]["cut"]]      # for AsrDatamodule
...
def main():
    ...
    # asr_datamodule = FinetuneAsrDataModule(args)

    test_sets_cuts = asr_datamodule.test_cuts()
