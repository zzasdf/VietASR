...
def greedy_search(
    ...
    blank_decoder_out, (blank_h, blank_c) = model.blank_decoder(sos)
    vocab_decoder_out, (vocab_h, vocab_c) = model.vocab_decoder(sos)
    ...
    # max_sym_per_frame = 3     # default maximum symbol per frame equals 1
    ...
    while t < T and sym_per_utt < max_sym_per_utt:
        ...
        logits, _ = model.joiner(
            current_encoder_out, blank_decoder_out, vocab_decoder_out
        )
        ...
        if y != blank_id:
            hyp.append(y.item())
            y = y.reshape(1, 1)
            blank_decoder_out, (blank_h, blank_c) = model.blank_decoder(
                y, (blank_h, blank_c)
            )
            vocab_decoder_out, (vocab_h, vocab_c) = model.vocab_decoder(
                y, (vocab_h, vocab_c)
            )
...
def greedy_search_batch(
    ...
    blank_id = model.vocab_decoder.blank_id
    ...
    N = encoder_out.size(0)     # batch size
    ...
    blank_decoder_out, (blank_h, blank_c) = model.blank_decoder(decoder_input)
    vocab_decoder_out, (vocab_h, vocab_c) = model.vocab_decoder(decoder_input)

    offset = 0
    for t, batch_size in enumerate(batch_size_list):
        start = offset
        end = offset + batch_size
        current_encoder_out = packed_encoder_out.data[start: end]    # packed into (sum_lengths, dim)
        current_encoder_out = current_encoder_out.unsqueeze(1)
        # current_encoder_out's shape: (batch_size, 1, encoder_out_dim)

        assert current_encoder_out.shape[0] == batch_size, current_encoder_out.shape
        offset = end

        blank_decoder_out = blank_decoder_out[:batch_size]
        vocab_decoder_out = vocab_decoder_out[:batch_size]
        ...
        if emitted:
            ...
            # decoder_input = [h[-1:] for h in hyps[:batch_size]]
            decoder_input = [h[-1:] for h in hyps]      # we dont constrain to batchsize here
            decoder_input = torch.tensor(
                decoder_input,
                device=device,
                dtype=torch.int64,
            )
            blank_decoder_out, (blank_h, blank_c) = model.blank_decoder(decoder_input, (blank_h, blank_c))
            vocab_decoder_out, (vocab_h, vocab_c) = model.vocab_decoder(decoder_input, (vocab_h, vocab_c))
