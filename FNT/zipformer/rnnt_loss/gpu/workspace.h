...
#include "options.h"
#include "assert.h" 
// CHECK_XX is not defined, use assert
...
class DtypeWorkspace 
{
 public:
  ...
  static int ComputeSizeFromOptions(const Options& options)
  {
    ...
    assert(options.device_ != UNDEFINED);
 ...
 private:
  static int ComputeSizeForDenominators(const Options& options) 
  { // B * T * U
    return options.BTU();
  }

  static int ComputeSizeForLogProbs(const Options& options) 
  { // B * T * U * 2
    return options.BTU() * 2;
  }

  static int ComputeSizeForAlphas(const Options& options) 
  { // B * T * U
    return options.BTU();
  }

  static int ComputeSizeForBetas(const Options& options) 
  { // B * T * U
    return options.BTU();
  }
...
class IntWorkspace 
{
 ...
 private:
  ...
  static int ComputeSizeForAlphaCounters(const Options& options) 
  { // B * U
#ifdef USE_CUDA
  ...
  static int ComputeSizeForBetaCounters(const Options& options) 
  { // B * U
#ifdef USE_CUDA
    if (options.device_ == GPU) 
    {
