...
from sigmoid_attention_joiner import SigmoidAttentionJoiner
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    # All-in-One ASR arguments
    parser.add_argument(
        "--use-lm-mode",
        type=str2bool,
        default=True,
        help="If True, use LM mode training for native ILME.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Scale for LM loss in All-in-One training.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of attention heads in SigmoidAttentionJoiner.",
    )
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    """Get joiner model - uses SigmoidAttentionJoiner for All-in-One ASR."""
    joiner = SigmoidAttentionJoiner(
        encoder_dim=max(_to_int_tuple(params.encoder_dim)),
        decoder_dim=params.decoder_dim,
        joiner_dim=params.joiner_dim,
        vocab_size=params.vocab_size,
        num_heads=params.joiner_num_heads,
    )
    return joiner
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        # All-in-One: LM loss for native ILME training
        lm_loss = torch.tensor(0.0, device=device)
        if params.use_lm_mode and params.use_transducer and is_training:
            # Get model module
            m = model.module if isinstance(model, DDP) else model
            blank_id = m.decoder.blank_id
            
            # Get decoder output
            from icefall.utils import add_sos
            sos_y = add_sos(y.to(device), sos_id=blank_id)
            sos_y_padded = sos_y.pad(mode="constant", padding_value=blank_id)
            # sos_y_padded shape: (batch, max_len+1) - has SOS at beginning
            
            decoder_out = m.decoder(sos_y_padded)
            # decoder_out shape: (batch, max_len+1, decoder_dim)
            
            # Project and compute LM logits with mode='lm' (encoder zeroed)
            dec_proj = m.joiner.decoder_proj(decoder_out)
            enc_zero = torch.zeros_like(dec_proj)
            lm_logits = m.joiner(enc_zero, dec_proj, project_input=False, mode="lm")
            # lm_logits shape: (batch, max_len+1, vocab_size)
            
            # Target: y_padded (without SOS)
            y_padded = y.to(device).pad(mode="constant", padding_value=0)
            # y_padded shape: (batch, max_len)
            
            # lm_logits[:, :-1] predicts y_padded[:, :]
            # i.e., lm_logits at position i predicts target at position i
            lm_logits_shifted = lm_logits[:, :-1, :]  # (batch, max_len, vocab)
            
            # Create mask for valid positions
            row_splits = y.shape.row_splits(1)
            y_lens = (row_splits[1:] - row_splits[:-1]).to(device)
            batch_size, max_len = y_padded.shape
            mask = torch.arange(max_len, device=device).unsqueeze(0) < y_lens.unsqueeze(1)
            
            # Flatten for cross entropy
            lm_logits_flat = lm_logits_shifted.reshape(-1, lm_logits_shifted.size(-1))
            targets_flat = y_padded.reshape(-1)
            mask_flat = mask.reshape(-1)
            
            import torch.nn.functional as F
            lm_loss = F.cross_entropy(
                lm_logits_flat[mask_flat],
                targets_flat[mask_flat].long(),
                ignore_index=blank_id,
                reduction="sum",
            )
            
            loss += params.lm_loss_scale * lm_loss
    ...
    if params.use_lm_mode and params.use_transducer:
