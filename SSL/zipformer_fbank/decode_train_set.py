...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="""Specific the index of split""",
    )
...
def main():
    ...
    params.res_dir = params.exp_dir / "decode_train_set"
    ...
    if not params.use_averaged_model:
        if params.iter > 0:
            filenames = find_checkpoints(params.checkpoint_dir, iteration=-params.iter)[
                : params.avg
            ]
            if len(filenames) == 0:
                raise ValueError(
            ...
            load_checkpoint(f"{params.checkpoint_dir}/epoch-{params.epoch}.pt", model)
        else:
            start = params.epoch - params.avg + 1
            filenames = []
            for i in range(start, params.epoch + 1):
                if i >= 1:
                    filenames.append(f"{params.checkpoint_dir}/epoch-{i}.pt")
            logging.info(f"averaging {filenames}")
            model.to(device)
            model.load_state_dict(average_checkpoints(filenames, device=device))
    else:
        if params.iter > 0:
            ...
            filename_start = f"{params.checkpoint_dir}/epoch-{start}.pt"
            filename_end = f"{params.checkpoint_dir}/epoch-{params.epoch}.pt"
            logging.info(
    ...
    # print([args.cut_name])
    if args.cut_list is not None:
        with open(args.cut_list, 'r') as f:
            cuts_name = f.read().splitlines()
        test_sets = [f"{os.path.splitext(os.path.basename(args.cut_list))[0]}"]
    else:
        cuts_name = [args.cut_name]
        test_sets = [f"{os.path.splitext(os.path.basename(args.cut_name))[0]}"]

    test_cuts = [load_manifest_lazy(item) for item in cuts_name ]

    # def remove_short_and_long_utt(c):
    #     return (c.duration <= 20.0) and (c.duration>=0.5)  and c.supervisions[0].language=="Chinese (China)"

    # test_cuts = test_cuts.filter(remove_short_and_long_utt)

    # test_dl = finetune_datamoddule.test_dataloaders(test_cuts)
    ...
    # test_sets = [f"test_{args.split_index}_{args.split_n}"]
    test_dl = [finetune_datamoddule.test_dataloaders(cut) for cut in test_cuts]
