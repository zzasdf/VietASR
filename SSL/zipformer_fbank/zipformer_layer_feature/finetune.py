...
from zipformer_layer_feature.hubert_ce import HubertModel
from joiner import Joiner
from lhotse.cut import Cut
from lhotse.dataset.sampling.base import CutSampler
from lhotse.utils import fix_random_seed
from zipformer_layer_feature.model import AsrModel
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="whether to use final downsample",
    )
...
def get_parser():
    ...
    parser.add_argument(
        ...
        It specifies the directory where the pretrained checkpoint is saved. When it is a file, use a single checkpoint""",
    )
...
def get_encoder_model_checkpoint(params: AttributeDict):
    logging.info(f"Loading {params.pretrained_dir}")
    if os.path.isdir(params.pretrained_dir):
        pretrained = get_avg_checkpoint(
            params.pretrained_dir,
            params.pretrained_epoch,
            params.pretrained_avg,
            params.pretrained_use_averaged_model
            )
    else:
        pretrained = torch.load(params.pretrained_dir, map_location=torch.device("cpu"))

    if params.pretrained_type == "finetune":
        new_pretrained = OrderedDict()
        for item in pretrained:
            if item.startswith("encoder."):
                _, new_item = item.split(".", maxsplit=1)
                new_pretrained[new_item] = pretrained[item]
        pretrained = new_pretrained
    elif params.pretrained_type == "ASR":
        new_pretrained = OrderedDict()
        for item in pretrained:
            if item.startswith("encoder.") or item.startswith("encoder_embed."):
                new_pretrained[item] = pretrained[item]
        pretrained = new_pretrained

    if params.final_downsample:
        # avoid the miss match between pretrain and finetune
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    if hasattr(params, "pretrained_dir") and params.pretrained_dir is not None:
        logging.info(f"Loading {params.pretrained_dir}")
        pretrained = get_encoder_model_checkpoint(params)
        # pretrained = torch.load(params.pretrained_dir, map_location=torch.device("cpu"))
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    encoder_dim = max(_to_int_tuple(params.encoder_dim)) if params.encoder_feature_layer<0 else _to_int_tuple(params.encoder_dim)[params.encoder_feature_layer]
    joiner = Joiner(
...
def get_model(params: AttributeDict) -> nn.Module:
    ...
    model = AsrModel(
        ...
        encoder_feature_layer=params.encoder_feature_layer,
    )
    return model
