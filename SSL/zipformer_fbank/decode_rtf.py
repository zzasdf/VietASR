...
def clean_text_custom(text: str) -> str:
    # Chuyển về chữ hoa để xử lý đồng nhất (vì logic bên dưới dùng .upper())
    text = text.upper()
    
    # Loại bỏ 111 đứng riêng lẻ
    text = re.sub(r'\b111\b', '', text)
    ...
    for batch_idx, batch in enumerate(dl):
        ...
        for name, hyps in hyps_dict.items():
            ...
            for cut_id, hyp_words, ref_text in zip(cut_ids, hyps, texts):
                clean_ref = clean_text_custom(ref_text)
                ref_words = clean_ref.upper().split()

                # 2. Làm sạch kết quả dự đoán (Hypothesis)
                # hyp_words đang là List[str], cần nối lại để regex, rồi tách ra
                hyp_str = " ".join(hyp_words)
                clean_hyp = clean_text_custom(hyp_str)
                hyp_words_clean = clean_hyp.split()
