...
def get_avg_1_checkpoint(epoch, exp_dir, device):
    return average_checkpoints_with_averaged_model(
        filename_start=f"{exp_dir}/epoch-{epoch-1}.pt",
        filename_end=f"{exp_dir}/epoch-{epoch}.pt",
        device=device,
    )

def average_filter_checkpoints(
    epoch_list: List[Path], exp_dir, device: torch.device = torch.device("cpu")
) -> dict:
    ...
    avg = get_avg_1_checkpoint(epoch_list[0], exp_dir, device)
    ...
    for i in range(1, n):
        state_dict = get_avg_1_checkpoint(epoch_list[i], exp_dir, device)
...
def main():
    ...
    params.res_dir = params.exp_dir/ "filter_decode" / params.decoding_method
    ...
    if not params.use_averaged_model:
        ...
        if params.iter > 0:
            ...
            with open(params.filter_list, 'r') as f:
                line = f.read()
            line = line.split()
            line = [int(item) for item in line]

            start = params.epoch - params.avg + 1
            epoch_list = line[start: params.epoch+1]
            # logging.info(f"averaging {filenames}")
            model.to(device)
            model.load_state_dict(average_filter_checkpoints(epoch_list, params.exp_dir, device=device))
