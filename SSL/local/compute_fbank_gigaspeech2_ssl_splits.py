...
def compute_fbank_gigaspeech2_ssl_splits(args):
    ...
    for i in range(start, stop):
        ...
        cuts_path = output_dir / f"gigaspeech2-ssl_cuts_{args.dataset}.{idx}.jsonl.gz"
        # if cuts_path.is_file():
        #     logging.info(f"{cuts_path} exists - skipping")
        #     continue

        raw_cuts_path = (
            output_dir / f"gigaspeech2-ssl_cuts_{args.dataset}_raw.{idx}.jsonl.gz"
        )
        ...
        cut_set = cut_set.compute_and_store_features_batch(
            extractor=extractor,
            storage_path=f"{output_dir}/gigaspeech2-ssl_feats_{idx}",
...
def main():
    ...
    if task == "run":
        ...
        compute_fbank_gigaspeech2_ssl_splits(args)
    elif task == "parallel":
...
def run(src_dir, dataset, index, lock_file_name):
    ...
    state = os.system(f"CUDA_VISIBLE_DEVICES={device} PYTHONUTF8=1 python3 ./local/compute_fbank_gigaspeech2_ssl_splits.py run --src-dir {src_dir} --dataset {dataset} --num-workers 2 --start {index} --stop {index+1} --batch-duration 1000 --num-splits {index+1} 2>&1 | tee log_tem/{dataset}_{index}.log")
    # state = 0
    with device_lock:
        with open(lock_file_name,'r') as f:
            line = f.read().strip()
