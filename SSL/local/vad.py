...
def routine(wav_file, tgt_dir, model, device, args):
    ...
    res[0]["value"] = merge_segment(res[0]["value"], args.merge_duration, args.min_duration, args.max_blank_duration)
...
def main(rank, args, task_lines):
    ...
    model = AutoModel(model="fsmn-vad", model_revision="v2.0.4", device=f"cuda:{rank%num_gpus}", max_end_silence_time=500)
    # model.to(device)
